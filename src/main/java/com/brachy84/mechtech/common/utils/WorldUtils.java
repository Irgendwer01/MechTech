...
public class WorldUtils {
    ...
    public static List<BlockPos> getBlockBox(BlockPos center, int range) {
        ...
        return getBlockBox(center.getX() - range, center.getY() - range, center.getZ() - range, center.getX() + range,
                center.getY() + range, center.getZ() + range);
    }

    public static List<BlockPos> getBlockBox(BlockPos posNeg, BlockPos posPos) {
        return getBlockBox(Math.min(posNeg.getX(), posPos.getX()), Math.min(posNeg.getY(), posPos.getY()),
                Math.min(posNeg.getZ(), posPos.getZ()), Math.max(posNeg.getX(), posPos.getX()),
                Math.max(posNeg.getY(), posPos.getY()), Math.max(posNeg.getZ(), posPos.getZ()));
    }
