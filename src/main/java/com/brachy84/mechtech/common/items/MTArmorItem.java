...
public class MTArmorItem extends ArmorMetaItem<ArmorMetaItem<?>.ArmorMetaValueItem> {
    ...
    public void registerSubItems() {
        MODULAR_HELMET = addItem(0, "modular_helmet")
                .setArmorLogic(new ModularArmor(EntityEquipmentSlot.HEAD, MTConfig.modularArmor.helmetSlots, 8000));
        MODULAR_CHESTPLATE = addItem(1, "modular_chestplate").setArmorLogic(
                new ModularArmor(EntityEquipmentSlot.CHEST, MTConfig.modularArmor.chestPlateSlots, 512000));
        MODULAR_LEGGINGS = addItem(2, "modular_leggings")
                .setArmorLogic(new ModularArmor(EntityEquipmentSlot.LEGS, MTConfig.modularArmor.leggingsSlots, 64000));
        MODULAR_BOOTS = addItem(3, "modular_boots")
                .setArmorLogic(new ModularArmor(EntityEquipmentSlot.FEET, MTConfig.modularArmor.bootsSlot, 0));
    }
    ...
    public void addInformation(@Nonnull ItemStack itemStack, @Nullable World worldIn, @Nonnull List<String> lines,
        ...
        if (item != null) {
            ...
            if (!modules.isEmpty()) {
                lines.add(I18n.format("metaitem.modular_armor.installed_modules", modules.size(),
                        modularArmor.getModuleSlots()));
            ...
            if (electricItem != null) {
                if (electricItem.getMaxCharge() == 0) {
                    ...
                    lines.add(I18n.format("metaitem.generic.electric_item.tooltip", electricItem.getCharge(),
                            electricItem.getMaxCharge(), GTValues.VN[electricItem.getTier()]));
                }
            }
